<h1 align="center">🥚 洛克王国 · 异色生蛋智能配窝助手</h1>

<p align="center">
  <strong>杜绝雄性闲置 · 唯一依赖优先 · 均衡覆盖</strong><br>
  纯前端工具，帮你自动生成最优配窝方案，让每一窝都出蛋。
</p>

<p align="center">
  <a href="https://heartsarsonist.github.io/rocokingdom_egg/">🌐 在线演示</a> ·
  <a href="https://www.bilibili.com/video/BV1X79oB1ETW/?spm_id_from=333.1387.homepage.video_card.click&vd_source=e16eca063102daa8013a6331d9d70109">🎬 B站视频教程</a> ·
</p>

---

## 🎬 B站视频教程
[点击观看完整演示与讲解](https://www.bilibili.com/video/BV1X79oB1ETW/?spm_id_from=333.1387.homepage.video_card.click&vd_source=e16eca063102daa8013a6331d9d70109)

---

## ✨ 特性

- **3 步出方案**：设置窝数 → 添加精灵 → 一键生成，全程无需心算。
- **唯一依赖智能识别**：自动锁定只能与单一雄性配对的雌性，避免遗漏。
- **可视化窝位布局**：7×7 网格 + 曼哈顿距离约束（≤2步），支持拖拽微调。
- **杜绝雄性闲置**：算法自动精简多余雄性，确保每一只都发挥价值。
- **一键导出长图**：方案详情与精灵窝位置均可截屏导出，方便分享。
- **完全本地运行**：纯静态页面，无隐私风险；宠物列表、兼容表通过 `egg.csv` 自定义。
- **零依赖部署**：一个 HTML + 一个 CSV，扔到任意静态服务器或本地打开即用。

---

## 🚀 快速开始


git clone https://github.com/heartsarsonist/rockingdom_egg.git
cd rockingdom_egg
# 直接用浏览器打开 index.html，或部署到任意静态服务（如 GitHub Pages）

**使用流程：**

1. 设置精灵窝数量（上限 10，自动保留至少 1 个雄性窝）。
2. 点击“添加雌性精灵”选择需要孵蛋的宠物，数量不超过 `窝数-1`。
3. 点击“添加雄性精灵”输入你实际拥有的雄性库存。
4. 点击 **✨ 智能推荐配窝方案**，立即获得最优雄性分配与覆盖详情。
5. 点击 **📍 生成位置图** 查看 7×7 放置方案，可拖拽调整精灵位置，并导出图片。

---

## ⚖️ 方案对比

| 指标 | 手动心算 | 本助手 |
|------|----------|--------|
| 耗时 | 10~30 分钟 | **< 10 秒** |
| 雄性闲置概率 | 高（经常多放无用雄性） | **0**（自动优化） |
| 唯一依赖遗漏 | 经常发生 | **自动锁定并高亮** |
| 出错后调整 | 推倒重来 | **拖拽即可** |

---
## 🧠 算法说明

### 1. 智能雄性推荐

- **唯一依赖优先**：若某只雌性只能与唯一一种雄性生蛋，则强制锁定该雄性，避免遗漏配对。
- **贪婪覆盖**：剩余雄性名额通过贪心算法分配，优先选择能覆盖最多未覆盖雌性的雄性。
- **防闲置机制**：雄性总数不超过可配对雌性种类数，杜绝“有窝无蛋”的浪费。

### 2. 曼哈顿距离布局

- 精灵窝分布在 7×7 网格上，配对有效的距离为曼哈顿距离 ≤ 2 步。
- 生成位置时优先保证“唯一依赖”的雌性与锁定雄性紧邻（距离=1）。
- 使用随机尝试 + 回溯搜索找到满足所有约束的布局，并自动居中、紧凑化。

---

## 🛠 技术栈

- 纯 HTML/CSS/JS，无框架
- SVG 动态绘图 + 拖拽交互
- html2canvas 导出图片
- BFS 算法求解网格最短路径与位置约束
## 📁 自定义宠物与兼容表

根目录下的 `egg.csv` 为宠物兼容矩阵，格式如下：

- 第一行为标题行：首列留空，后续列为**宠物名**。
- 之后每一行第一列为宠物名，与该列宠物可生蛋则填 `1`，否则填 `0`。
- 修改后**无需重新编译**，刷新页面即可生效。

```csv
,燃薪虫,双灯鱼,月牙雪熊
燃薪虫,1,0,0
双灯鱼,0,1,0
月牙雪熊,0,0,1
